...
Некоторые каналы предоставляют функции, которые невозможно реализовать, используя только [текст сообщений и вложения](../dotnet/bot-builder-dotnet-create-messages.md). Чтобы реализовать функции, связанные с каналами, вы можете передать в канал собственные метаданные через свойство `ChannelData` объекта `Activity`. Например, с помощью свойства `ChannelData` бот может передать в Telegram команду отправки наклейки или потребовать, чтобы Office 365 отправил сообщение электронной почты.
...
> [!NOTE]
> Значение свойства `ChannelData` объекта `Activity` — это объект JSON. Поэтому в примерах в этой статье показан ожидаемый формат свойства JSON `channelData` в различных сценариях. Чтобы создать объект JSON с помощью .NET, используйте класс `JObject` (.NET). 
...
> [!NOTE]
> Сообщения, получаемые ботом от пользователей по каналу электронной почты, могут содержать свойство `ChannelData`, которое содержит объект JSON, как описано выше.
...
Бот может ответить на это сообщение [обычным способом](../dotnet/bot-builder-dotnet-connector.md#create-reply) или отправить ответ напрямую на конечную точку, которая определена свойством `response_url` объекта `payload`.
...
Чтобы создать динамические кнопки, можете использовать следующий код:
```cs
...
Чтобы создать интерактивные меню, используйте такой код:
```cs
...
- [Общие сведения о действиях](../dotnet/bot-builder-dotnet-activities.md)
- [Создание сообщений](../dotnet/bot-builder-dotnet-create-messages.md)
