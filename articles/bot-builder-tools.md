...
description: Средства Bot Framework позволяют управлять ресурсами бота непосредственно из командной строки
...
ms.locfileid: "54453859"
---
# <a name="manage-bots-using-cli-tools"></a>Управление ботами с помощью средств CLI

Средства Bot Framework позволяют выполнять весь рабочий процесс разработки чат-ботов, включая планирование, сборку, тестирование, публикацию, подключение и оценку. Давайте узнаем, как эти средства помогут вам на каждом из этапов цикла разработки.
...
После определения компонентов языка LUIS или QnA в формате LU их можно опубликовать в файле LUIS.json, QnA.json или QnA.tsv. При запуске средство LUDown ищет в рабочей папке любые файлы для анализа с форматом LU. Так как для работы с LU-файлами средство LUDown поддерживает и LUIS, и QnA, нам просто нужно указать соответствующую языковую службу. Для этого можно использовать команду **ludown parse \<to-service-type> -- in \<lu-file-path>**.
...
Bot Framework [Emulator](bot-service-debug-emulator.md) — это классическое приложение, которое позволяет разработчикам локально или удаленно (через туннель) тестировать боты и (или) выполнять их отладку.

## <a name="publish"></a>Опубликовать

Вы можете создавать, скачивать и публиковать боты в службе Azure Bot с помощью Azure CLI.

Для работы с msbot 4.3.2 и более поздними версиями вам понадобится Azure CLI версии 2.0.53 или более поздней. Если вы установили расширение botservice, удалите его с помощью следующей команды.
...
Затем создайте ресурс бота, в котором вы будете публиковать этот бот.
...
- [Средства Bot Framework на сайте GitHub][cliTools]
- [.lu File Format](https://aka.ms/ludown-file-format) (Формат файлов LU)
