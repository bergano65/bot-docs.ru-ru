---
title: Обработка действий пользователя — Служба Azure Bot
...
| Действие | Область | Description |
|------|------| ---- |
| [triggerAction](#bind-a-triggeraction) | Global | Привязывает к диалогу действие, которое очистит стек диалогов и поместит диалог в нижнюю часть стека. Используйте параметр `onSelectAction`, чтобы переопределить это поведение по умолчанию. |
| [customAction](#bind-a-customaction) | Global | Привязывает к боту настраиваемое действие, которое может обрабатывать сведения или выполнять действие, не влияя на стек диалогов. Используйте параметр `onSelectAction`, чтобы настроить функциональные возможности данного действия. |
