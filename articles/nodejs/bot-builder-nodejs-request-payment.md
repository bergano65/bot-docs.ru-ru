...
Если ваш бот позволяет пользователям приобретать позиции, он делает запрос платежей, включая специальный тип кнопки в [форматированной карточке](bot-builder-nodejs-send-rich-cards.md). Эта статья описывает отправку запроса платежей с помощью пакета SDK Bot Builder для Node.js.
...
2. <a href="https://seller.microsoft.com/en-us/dashboard/registration/seller/?accountprogram=botframework" target="_blank">Войдите в Центр продавца, используя свою учетную запись Майкрософт</a>.

3. В Центре продавца подключите свою учетную запись с помощью Stripe.
...
![Примеры бота для оплаты](../media/payments-bot-buy.png) 
...
| `value` | Полезная нагрузка запроса в формате JSON. | 
