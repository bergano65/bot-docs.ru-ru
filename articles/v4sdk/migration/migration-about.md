...
При создании адаптера для бота вы также указываете делегата обработчика шагов, который будет получать входящие действия от каналов и пользователей. Адаптер создает объект контекста шага для каждого полученного действия. Он передает объект контекста шага обработчику шагов, а затем удаляет объект после завершения шага.

Обработчик шагов может получать действия разных типов. Как правило, диалогам в боте необходимо перенаправлять только действия _сообщений_. Дополнительные сведения о типах действий см. в статье о [схеме действий](https://aka.ms/botSpecs-activitySchema).

### <a name="handling-turns"></a>Шаги обработки

Ваш обработчик шагов должен соответствовать сигнатуре для `BotCallbackHandler`:
...
При обработке шага используйте контекст шага, чтобы получить данные о входящем действии и отправить действия пользователю:

| | |
|-|-|
| Получение входящего действия | Получите свойство `Activity` контекста шага. |
| Создание действия и отправка его пользователю | Вызовите метод `SendActivityAsync` контекста шага.<br/>Дополнительные сведения см. в статьях [Отправка и получение текстовых сообщений](../bot-builder-howto-send-messages.md) и [Добавление мультимедиа в сообщения](../bot-builder-howto-add-media-attachments.md). |
...
Дополнительные сведения см. в разделе [Сохранение состояния](../bot-builder-concept-state.md#saving-state).
...
- Каскадный диалог, экземпляр класса `WaterfallDialog`.

  Разработан для оптимальной работы с диалогами запроса, которые отправляют запрос на получение различных вводных данных от пользователя и проверяют их. См. статью [Сбор данных, которые вводит пользователь, с помощью диалогового окна](../bot-builder-prompts.md).

  Это автоматизирует большинство процессов, но требует применения определенной формы для кода диалога (см. статью [Реализация последовательной беседы](../bot-builder-dialog-manage-conversation-flow.md)). Но вы можете создать другие потоки управления, добавив несколько диалогов в набор диалогов (см. статью [Создание сложного потока беседы с использованием ветвления и циклов](../bot-builder-dialog-manage-complex-conversation-flow.md)).

- Диалог компонента, полученный из класса `ComponentDialog`.

  Позволяет вам инкапсулировать код диалога, не вызывая конфликты именования с другими контекстами. См. статью [Повторное использование диалогов](../bot-builder-compositcontrol.md).

- Настраиваемый диалог, полученный из абстрактного класса `Dialog`.

  Дает максимальную гибкость в аспекте поведения диалогов, но также требует более глубоких знаний о реализации стека диалогов.

Чтобы получить доступ к диалогу, вам необходимо разместить его экземпляр в _наборе диалогов_, а затем сгенерировать _контекст диалога_ для такого набора.

При создании набора диалогов вам нужно указать метод доступа к свойству состояния диалога. Это позволит платформе сохранять состояние диалога при переходе между шагами. Управление состоянием в версии 4 описано в статье [Управление состоянием](../bot-builder-concept-state.md).
...
- Различные производные классы `Prompt` в версии 4 реализуют запросы пользователей в виде отдельных диалогов из двух шагов. Узнайте, как [собирать данные, которые вводит пользователь, с помощью диалогового окна](../bot-builder-prompts.md).
