...
- [Базовые знания о ботах](bot-builder-basics.md), [LUIS][howto-luis], and [QnA Maker][howto-qna]. 
- [Средство подготовки к отправке](https://github.com/Microsoft/botbuilder-tools/tree/master/packages/Dispatch)
- Пример **NLP with Dispatch** из репозитория кода для [C#][cs-sample]or [JS Sample][js-sample].
...
Средству отправки требуется доступ для чтения имеющихся приложений LUIS и QnA Maker для создания родительского приложения LUIS, которое отправляется приложениям LUIS и QnA Maker. Вместе с этим доступом предоставляются идентификаторы приложений и ключи разработки. Вам нужен идентификатор и ключ для каждого приложения LUIS и приложения QnA Maker.

|Приложение|Расположение сведений|
|--|--|
|LUIS|Идентификатор для каждого приложения находится на [портале LUIS](https://www.luis.ai). Для его получения выберите "Управление -> Сведения о приложении".<br>Ключ авторизации находится на портале LUIS, в правом верхнем углу. Выберите пользователя, а затем "Параметры".|
|QnA Maker| Идентификатор приложения находится на портале [QnA Maker](https://http://qnamaker.ai) на странице параметров, доступной после публикации приложения. Это идентификатор, расположенный в первой части команды POST после элемента knowledgebase. Например, `POST /knowledgebases/{APP-ID}/generateAnswer`.<br>Ключ разработки находится на портале Azure для ресурса QnA Maker в разделе **Ключи**. Вам потребуется только один ключ.|
