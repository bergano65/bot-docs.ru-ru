...
description: Как автоматически перевести введенные пользователем данные на родной язык бота, а затем обратно на язык пользователя.
...
ms.locfileid: "39301514"
---
# <a name="translate-user-input-to-make-your-bot-multilingual"></a>Перевод введенных пользователем данных для создания бота с поддержкой нескольких языков

[!INCLUDE [pre-release-label](../includes/pre-release-label.md)]

Ваш бот может использовать [Microsoft Translator](https://www.microsoft.com/en-us/translator/), чтобы автоматически переводить сообщения на язык, который понимает ваш бот, и при необходимости переводить ответы бота обратно на язык пользователя. Добавление возможности перевода для вашего бота позволяет охватывать более широкую аудиторию без изменений значительных частей основной программы бота.
<!-- 
...
* `Microsoft.Bot.Builder.Ai.Translation` (необходимо для перевода)

Если вы собираетесь объединить перевод со службой "Распознавание речи" (LUIS), необходимо также добавить ссылку на:

* `Microsoft.Bot.Builder.Ai.Luis` (необходимо для LUIS)
...
> [!TIP] 
> Пакет SDK для Bot Builder автоматически определяет язык пользователя на основе только что отправленных сообщений. Чтобы переопределить эту функцию, вы можете указать дополнительные параметры обратного вызова, чтобы добавить собственную логику для определения и изменения языка пользователя.  



Взгляните на код в файле `EchoBot.cs`, где бот отправляет высказывание "Вы отправили", за которым следует сообщение пользователя:
...
При добавлении ПО промежуточного слоя для перевода необязательный параметр указывает, следует ли переводить ответы обратно на язык пользователя. В `Startup.cs` мы указали `false`, чтобы просто перевести сообщения пользователя на язык бота.
...
Запустите бот и напечатайте несколько сообщений на других языках. Вы увидите, что бот перевел сообщение пользователя и указывает перевод в своем ответе.

![Бот определяет язык и переводит входные данные](./media/how-to-bot-translate/bot-detects-language-translates-input.png)




## <a name="invoke-logic-in-the-bots-native-language"></a>Вызов логики на родном языке бота
...
Кроме того, можно перевести ответы на язык пользователя, установив последнему параметру конструктора значение `true`.
...
Запустите бот и напечатайте несколько сообщений на других языках. Вы увидите, что он определяет язык пользователя и переводит ответ.

![Бот определяет язык и переводит ответ](./media/how-to-bot-translate/bot-detects-language-translates-response.png)
...
## <a name="combining-luis-or-qna-with-translation"></a>Объединение LUIS или QnA с переводом

При объединении перевода с другими службами в боте, например LUIS или QnA Maker, сначала добавьте ПО промежуточного слоя для перевода, чтобы сообщения переводились перед их передачей до другого ПО промежуточного слоя, которое ожидает родной язык бота.
...
В коде бота результаты LUIS зависят от входных данных, которые уже были переведены на родной язык бота. Попробуйте изменить код бота, чтобы проверить результаты приложения LUIS:
...
Возможно, некоторые слова не нужно переводить, такие как имена собственные. Вы можете предоставить регулярные выражения, чтобы указать шаблоны, которые не следует переводить. Например, если пользователь скажет: "Меня зовут …" не на родном языке бота и нужно избежать перевода имени, можно использовать шаблон, чтобы указать это.
...
![Бот пропускает перевод шаблона](./media/how-to-bot-translate/bot-no-translate-name-fr.png)

## <a name="localize-dates"></a>Локализация дат

Если нужно локализовать даты, вы можете добавить `LocaleConverterMiddleware`. Например, если вы знаете, что ваш бот ожидает даты в формате `MM/DD/YYYY`, а пользователи с другими языковыми стандартами могут вводить даты в формате `DD/MM/YYYY`, локальный преобразователь ПО промежуточного слоя может автоматически преобразовывать даты в формат, который ожидает ваш бот.

> [!NOTE]
> Локальный преобразователь ПО промежуточного слоя призван преобразовывать только даты. Он не имеет сведений о результатах перевода ПО промежуточного слоя. Если вы используете ПО промежуточного слоя для перевода, будьте внимательны, каким образом вы объединяете его с локальным преобразователем. ПО промежуточного слоя для перевода переводит некоторые даты, которые вводятся в текстовом формате, но не переводит даты.

Например, на следующем рисунке изображен бот, который выводит обратно на экран входные данные пользователя после перевода с английского языка на французский. Он использует `TranslationMiddleware` без использования `LocaleConverterMiddleware`.

![Бот переводит даты без преобразования](./media/how-to-bot-translate/locale-date-before.png)

Ниже показан тот же бот, но с добавлением `LocaleConverterMiddleware`.

![Бот переводит даты без преобразования](./media/how-to-bot-translate/locale-date-after.png)
