...
Клиенты, которые не могут использовать WebSocket, или клиенты, которым требуется получить журнал общения, могут извлекать действия с помощью `HTTP GET`.
