...
Помимо стандартных заголовков HTTP-запроса запрос Direct Line API должен включать заголовок `Authorization`, указывающий секрет или токен для проверки подлинности клиента, который выполняет запрос. Можно указать заголовок `Authorization` с помощью схем "Bearer" или "BotConnector". 
...
Дополнительные сведения о том, как получить секрет или токен, который клиент может использовать для аутентификации запросов Direct Line API, см. в разделе [Проверка подлинности](bot-framework-rest-direct-line-1-1-authentication.md).
...
| 401 | Клиент не авторизован для выполнения запроса. Часто этот код состояния возникает, потому что заголовок `Authorization` отсутствует или имеет неправильный формат. |
...
| 404 | Запрошенный ресурс не найден. Обычно этот код состояния указывает недопустимый URI запроса. |
| 500 | В службе Direct Line возникла внутренняя ошибка сервера или произошел сбой в работе бота. Если при отправке сообщения боту появляется ошибка 500, возможно, она была вызвана сбоем в работе бота. **Это общая ошибка кода.** |

## <a name="token-operations"></a>Операции с токенами 
Используйте эти операции для создания или обновления токена, который клиент может использовать для доступа к одному общению.

| Операция | ОПИСАНИЕ |
|----|----|
| [Generate Token](#generate-token) | Создает токен для нового общения. | 
| [Refresh Token](#refresh-token) | Обновляет токен. | 

### <a name="generate-token"></a>Generate Token
Создает токен, допустимый для одного общения. 
```http 
...
| Операция | ОПИСАНИЕ |
|----|----|
| [Start Conversation](#start-conversation) | Открывает новое общение с ботом. | 
...
### <a name="start-conversation"></a>Start Conversation
Открывает новое общение с ботом. 
```http 
...
Определяет общение Direct Line.<br/><br/>

| Свойство | type | ОПИСАНИЕ |
|----|----|----|
| **conversationId** | строка | Идентификатор, который уникально идентифицирует общение, для которого указанный токен является допустимым. |
| **token** | строка | Токен, действительный для указанного общения. |
| **expires_in** | number | Количество секунд до истечения срока действия токена. |
