...
ms.locfileid: "52500653"
---
# <a name="virtual-assistant-solution-overview"></a>Общие сведения о решении виртуального помощника

## <a name="overview"></a>Обзор
Многие наши клиенты и партнеры хотели бы иметь помощника для ведения диалога, который бы представлял конкретную торговую марку, учитывал особенности целевой аудитории и был доступным для широкого ряда устройств и холстов диалога. В рамках подхода, примененного при разработке пакета SDK для Bot Framework с открытым кодом от Майкрософт, решение личного помощника предоставляет средства управления взаимодействием с пользователем на основе набора базовых возможностей. Элементы интерфейса могут дополняться сведениями о пользователе и любом устройстве или экосистеме, предоставляя целостное интеллектуальное решение.
...
В настоящее время мы тесно сотрудничаем с первыми клиентами и партнерами через репозиторий с открытым исходным кодом, чтобы реализовать основные возможности.
...
Виртуальный помощник создан на основе [корпоративного шаблона](./bot-builder-enterprise-template-overview.md), в котором предоставляются базовые возможности для ведения диалога, включая базовый набор намерений на нескольких языках, диспетчеризацию, QnA и анализ диалога. Ниже перечислены возможности помощника, которые предоставляются сейчас. Мы планируем пополнить этот список и будем тесно сотрудничать с клиентами и партнерами в вопросах информирования о стратегии развития.

Функция | ОПИСАНИЕ |
------------ | -------------
Переход | Пример последовательности для начала общения, который позволяет помощнику отправить приветственное сообщение пользователю и собирать начальные сведения.
...
Включение навыков | Сейчас большинство базовых функций разработчик должен создавать самостоятельно. Наше решение виртуального помощника включает новую функцию "Навык", которая позволяет подключать к личному помощнику дополнительные возможности путем изменения конфигурации. Также решение предоставляет механизм аутентификации для навыков, чтобы запрашивать маркеры для последующих действий.
...
Автоматизированное развертывание | Все ресурсы Azure, необходимые для помощника, развертываются автоматически: служба регистрации бота, Служба приложений Azure, LUIS, QnAMaker, Content Moderator, CosmosDB, служба хранилища Azure и Application Insights. Кроме того, создаются, обучаются и публикуются модели LUIS для навыков, QnAMaker и модели Dispatch, чтобы обеспечить оперативное тестирование.
...
Виртуальный помощник поддерживает широкий ряд отраслевых сценариев. Ниже в ознакомительных целях представлены некоторые примеры таких сценариев.
...
Решение "Виртуальный помощник" доступно в [репозитории GitHub](https://github.com/Microsoft/AI/tree/master/solutions/Virtual-Assistant), который регулярно обновляется командой разработчиков виртуального помощника. Более подробная документация доступна в том же репозитории, а сведения о проблемах и отзывы можно передать через стандартные механизмы обратной связи на сайте GitHub. Если у вас есть важный сценарий для обсуждения, заполните [эту форму](https://aka.ms/customassistantpreviewform), и мы постараемся связаться с вами.
