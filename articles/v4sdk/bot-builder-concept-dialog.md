...
При выполнении каскадных шагов сохраняется *контекст каскадного диалога*. Он применяется так же, как контекст диалога, то есть предоставляет сведения о контексте и состоянии текущего шага. Контекстный объект шага каскада используется для взаимодействия с набором диалогов из шага каскада.
...
Когда начинается новый диалог, он становится активным диалогом, перемещаясь на вершину стека. Он остается активным, пока не завершится, не будет удален с помощью метода [replace dialog](#repeating-a-dialog), или в стек не будет помещен новый диалог (из обработчика шагов или самим активным диалогом), который и станет новым активным диалогом. Когда новый активный диалог завершится, он будет извлечен из стека, и активным снова станет диалог, расположенный на предыдущем уровне стека. Этот механизм позволяет реализовать [ветвления и циклы](#looping-and-branching), о которых рассказывается ниже.
...
Чтобы начать диалог, передайте нужный идентификатор *dialogId* в метод *begin dialog*, *prompt* или *replace dialog* контекста диалога. Метод begin dialog отправляет новый диалог на вершину стека, а метод replace dialog извлекает из стека текущий диалог и помещает на его место новый диалог.
...
Чтобы повторить диалог, используйте метод *replace dialog*. Метод контекста диалога *replace dialog* извлечет текущий активный диалог из стека (не выполняя для него нормальное завершение), поместит новый диалог на вершину стека и начнет его выполнение. Это отличный способ организовать [сложную итерацию](~/v4sdk/bot-builder-dialog-manage-complex-conversation-flow.md) или управление меню. Этот метод можно использовать для создания цикла, в котором диалог будет заменяться собой.

> [!NOTE]
> Если вам нужно сохранить внутреннее состояние текущего диалога, передайте сведения новому экземпляру диалога в вызове метода *replace dialog*, а затем соответствующим образом инициализируйте этот диалог. Параметры, передаваемые в новый диалог, можно получить из контекста шага с помощью свойства *options* на любом шаге диалога.
