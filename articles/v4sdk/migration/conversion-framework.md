...
ms.locfileid: "66215616"
---
# <a name="migrate-a-net-v3-bot-to-a-framework-v4-bot"></a>Перенос бота .NET версии 3 в бот .NET Framework версии 4
...
1. Обновите инструкции `using`: [!code-csharp[Using statements](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Global.asax.cs?range=4-13)].

1. Удалите следующие строки из метода **Application_Start**: [!code-csharp[Removed lines](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V3/ContosoHelpdeskChatBot/Global.asax.cs?range=23-24)].

    Теперь вставьте эту строку: [!code-csharp[Reference BotConfig.Register](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Global.asax.cs?range=22)].

1. Удалите метод **RegisterBotModules**, на который уже нет ссылок.

1. Замените метод **BotConfig.UpdateConversationContainer** представленным здесь методом **BotConfig.Register**, в котором мы регистрируем объекты, необходимые для поддержки внедрения зависимостей. Этот бот-помощник не использует состояние _пользователя_ или _приватной беседы_, поэтому мы создадим только объект управления состоянием беседы.
...
1. Обновите инструкции `using`: [!code-csharp[Using statements](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Controllers/MessagesController.cs?range=4-8)].

1. Удалите из класса атрибут `[BotAuthentication]`. В версии 4 проверку подлинности выполняет адаптер бота.

1. Добавьте эти поля и конструктор для инициализации. ASP.NET и Autofac используют внедрение зависимостей, чтобы получить значения параметров. (Чтобы поддерживать эту возможность, мы зарегистрировали объекты бота и адаптера в **Global.asax.cs**.) [!code-csharp[Fields and constructor](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Controllers/MessagesController.cs?range=14-21)]
...
1. Обновите инструкции `using`: [!code-csharp[Using statements](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Bots/DialogBot.cs?range=4-8)].
...
1. Переопределите `OnMessageActivityAsync` для вызова основного диалога. (Мы определим метод расширения `Run` чуть позже.) [!code-csharp[OnMessageActivityAsync](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Bots/DialogBot.cs?range=38-47)]
...
1. Обновите инструкции `using`: [!code-csharp[Using statements](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Dialogs/RootDialog.cs?range=4-10)].
...
1. Обновите инструкции `using`: [!code-csharp[Using statements](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Dialogs/InstallAppDialog.cs?range=4-11)].
...
1. В `LocalAdminPrompt` используйте этот код: [!code-csharp[Using statements](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Models/LocalAdminPrompt.cs?range=4)].

1. В `ResetPasswordPrompt` используйте этот код: [!code-csharp[Using statements](~/../botbuilder-samples/MigrationV3V4/CSharp/ContosoHelpdeskChatBot-V4NetFramework/ContosoHelpdeskChatBot/Models/ResetPasswordPrompt.cs?range=4-5)].
