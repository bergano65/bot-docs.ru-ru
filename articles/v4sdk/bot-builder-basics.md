...
| Полученное действие обновления диалога | `onConversationUpdate` | Вызывается при получении любого действия `conversationUpdate`. |
| Присоединение участников к беседе | `onMembersAdded` | Вызывается, когда к беседе присоединяются любые участники, включая самого бота. |
| Выход участников из беседы | `onMembersRemoved` | Вызывается, когда из беседы выходят любые участники, включая самого бота. |
| Получение действия реакции на сообщение | `onMessageReaction` | Вызывается при получении любого действия `messageReaction`. |
| Добавление реакции на сообщение в сообщение | `onReactionsAdded` | Вызывается, когда в сообщение добавляются реакции. |
| Удаление реакции на сообщение из сообщения | `onReactionsRemoved` | Вызывается, когда из сообщения удаляются реакции. |
| Полученное действие события | `onEvent` | Вызывается при получении любого действия `event`. |
| Полученное действие события ответа маркера | `onTokenResponseEvent` | Вызывается при получении события `tokens/response`. |
| Другой тип полученного действия | `onUnrecognizedActivityType` | Вызывается, если не определен обработчик для определенного типа действия. |
| Выполненные обработчики действий | `onDialog` | Вызывается после завершения всех применимых обработчиков. |

Вызовите параметр функции `next` из каждого обработчика, чтобы разрешить продолжение обработки. Если `next` не вызывается, обработка действия завершается.
