...
Убедитесь, что имеются пакеты, необходимые для добавления перевода к боту.
...
Можно настроить бот на вызов перевода для каждого сообщения, получаемого от пользователя. Для этого просто добавьте его в стек ПО промежуточного слоя бота. Чтобы изменить сообщение пользователя, используя окружающий контекст, в ПО промежуточного слоя используется результат перевода.
...
> [!TIP] 
> Пакет SDK для Bot Builder используется в только что отправленных сообщениях для автоматического определения языка пользователя. Чтобы переопределить эту функцию и добавить свою логику для определения и изменения языка пользователя, можно указать дополнительные параметры обратного вызова.  
...
Чтобы настроить ПО промежуточного слоя для перевода с помощью EchoBot, в app.js следует вставить следующий код.
...
Теперь добавьте логику, которая проверяет наличие слов на английском языке. Если пользователь ввел "help" или "cancel" на другом языке, бот переводит их на английский язык, а затем вызывает логику проверки для английских слов "help" или "cancel".
...
![Определение слова "Help", введенного на французском языке](./media/how-to-bot-translate/bot-detects-help-french.png)
...
Например, на следующем рисунке изображен бот, который выводит на экран входные данные пользователя, после того как он перевел их с английского на французский. Он использует только `TranslationMiddleware` (без `LocaleConverterMiddleware`).
